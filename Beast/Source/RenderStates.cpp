...
void RenderStates::Destroy( void )
{
	RELEASE( WireframeRS );
	RELEASE( NoCullRS );
	RELEASE( CullClockwiseRS );
	
	RELEASE( AlphaToCoverageBS );
	RELEASE( TransparentBS );
	RELEASE( NoRenderTargetWritesBS );
	
	RELEASE( MarkMirrorDSS );
	RELEASE( DrawReflectionDSS );
	RELEASE( NoDoubleBlendDSS );
	RELEASE( EqualsDSS );
}
