...
public:
	...
	std::vector<std::vector<XMFLOAT4X4>> Transforms;//, QuatTransforms;/** Array to return transformations results inside. */
};
...
public:
	...
	std::vector<cBone*>& GetBones() { return Bones; }
	...
	XMFLOAT4X4 GetBoneTransform(float dt, const std::string& bname) { int bindex=GetBoneIndex(bname); if(bindex == -1) return XMFLOAT4X4(); return Animations[CurrentAnimIndex].GetTransforms(dt)[bindex]; }
	// same as above, except takes the index
	XMFLOAT4X4 GetBoneTransform(float dt, unsigned int bindex) {  return Animations[CurrentAnimIndex].GetTransforms(dt)[bindex]; }
	...
	void ExtractAnimations(const aiScene* pScene, std::vector<std::string>& names);
...
protected:		
	...
	std::vector<XMFLOAT4X4> Transforms;// temp array of transfrorms
