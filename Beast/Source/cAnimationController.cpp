...
void TransformMatrix(XMFLOAT4X4& out,const aiMatrix4x4& in){// there is some type of alignment issue with my XMMATRIX and the aimatrix4x4 class, so the copy must be manually
	out._11=in.a1;
	out._12=in.a2;
	out._13=in.a3;
	out._14=in.a4;
...
void cAnimEvaluator::Evaluate( float pTime, std::map<std::string, cBone*>& bones) {
	...
	for( unsigned int a = 0; a < Channels.size(); a++){
		...
		//XMMATRIX tmp;// = XMLoadFloat4x4(&bonenode->second->LocalTransform);
		TransformMatrix(bonenode->second->LocalTransform, mat);
		//XMStoreFloat4x4(&bonenode->second->LocalTransform, tmp);
	}
	mLastTime = time;
}
...
void SceneAnimator::Init(const aiScene* pScene){// this will build the skeleton based on the scene passed to it and CLEAR EVERYTHING
	...
	name.push_back("BindPose");
	name.push_back("Idle");
	ExtractAnimations(pScene, name);
		...
		for (unsigned int n = 0; n < mesh->mNumBones;++n){
			...
			if(found != BonesByName.end()){// FOUND IT!!! woohoo, make sure its not already in the bone list
				...
				if(!skip){// only insert the bone if it has not already been inserted
					...
					//XMStoreFloat4x4(&found->second->Offset, tmp);
					XMStoreFloat4x4(&found->second->Offset, XMMatrixTranspose(XMLoadFloat4x4(&found->second->Offset)));// .Transpose();// transpoce their matrix to get in the correct format
	...
	for(size_t i(0); i< Animations.size(); i++){// pre calculate the animations
		...
		for(float ticks = 0; ticks < Animations[i].Duration; ticks += Animations[i].TicksPerSecond/30.0f){
			...
			for( size_t a = 0; a < Transforms.size(); ++a){
				...
				XMStoreFloat4x4(&rotationmat, XMLoadFloat4x4(&Bones[a]->Offset) * XMLoadFloat4x4(&Bones[a]->GlobalTransform));
...
void SceneAnimator::ExtractAnimations(const aiScene* pScene, std::vector<std::string>& names){
	//OUTPUT_DEBUG_MSG("Extracting Animations . . ");
	for(size_t i(0); i< pScene->mNumAnimations; i++){
		Animations.push_back(cAnimEvaluator(pScene->mAnimations[i]) );// add the animations
	}
	for(uint32_t i(0); i< names.size(); i++){// get all the animation names so I can reference them by name and get the correct id
		AnimationNameToId.insert(std::map<std::string, uint32_t>::value_type(names[i], i));
	}
	CurrentAnimIndex=0;
	SetAnimation("Idle");
}
...
void SceneAnimator::CalcBoneMatrices(){
	for( size_t a = 0; a < Transforms.size(); ++a){
		XMStoreFloat4x4(&Transforms[a], XMLoadFloat4x4(&Bones[a]->Offset) * XMLoadFloat4x4(&Bones[a]->GlobalTransform));
		/*
		XMMATRIX rotationmat = Transforms[a];
		quat q;
...
cBone* SceneAnimator::CreateBoneTree( aiNode* pNode, cBone* pParent){
	...
	//XMStoreFloat4x4(&internalNode->LocalTransform, tmp);
	XMStoreFloat4x4(&internalNode->LocalTransform, XMMatrixTranspose(XMLoadFloat4x4(&internalNode->LocalTransform)));// .Transpose();
...
void SceneAnimator::CalculateBoneToWorldTransform(cBone* child){
	...
	while( parent ){// this will climb the nodes up along through the parents concentating all the matrices to get the Object to World transform, or in this case, the Bone To World transform
		XMMATRIX tmp = XMLoadFloat4x4(&child->GlobalTransform) * XMLoadFloat4x4(&parent->LocalTransform);
		XMStoreFloat4x4(&child->GlobalTransform, tmp);// *= parent->LocalTransform;
